...
public class ASTRewriteCorrectionProposal extends CUCorrectionProposal {
     ...
     *         image is desired.
     */
    public ASTRewriteCorrectionProposal(String name, ASTRewrite rewrite, int relevance, Document document, Images image) {
    ...
    protected void addEdits(Document document, TextEdit editRoot) throws CoreException {
        ...
        if (rewrite != null) {
            try {
                TextEdit edit = rewrite.rewriteAST(document, WorkerMessageHandler.get().getOptions());
