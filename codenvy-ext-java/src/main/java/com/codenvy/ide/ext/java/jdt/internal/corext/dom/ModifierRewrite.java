...
public class ModifierRewrite {
     ...
     * @param editGroup
     *         the edit group in which to collect the corresponding text edits, or
    ...
    public void copyAllModifiers(ASTNode otherDecl, TextEditGroup editGroup, boolean copyIndividually) {
        ...
        List<ASTNode> originalList = modifierList.getOriginalList();
        if (originalList.isEmpty()) {
            return;
        }

        if (copyIndividually) {
            for (Iterator<ASTNode> iterator = originalList.iterator(); iterator.hasNext(); ) {
                ASTNode modifier = iterator.next();
            ...
            ASTNode copy =
                    modifierList.createCopyTarget((ASTNode)originalList.get(0),
                                                  (ASTNode)originalList.get(originalList.size() - 1));
            if (copy != null) { //paranoia check (only left here because we're in RC1)
                fModifierRewrite.insertLast(copy, editGroup);
            }
        }
    ...
    public void copyAllAnnotations(ASTNode otherDecl, TextEditGroup editGroup) {
        ...
        for (Iterator<ASTNode> iterator = originalList.iterator(); iterator.hasNext(); ) {
            ...
            if (modifier.isAnnotation()) {
                fModifierRewrite.insertLast(fModifierRewrite.getASTRewrite().createCopyTarget((Annotation)modifier),
                                            editGroup);
            }
        }
    }
     ...
     * @param modifiers
     *         the modifiers to set
     * @param consideredFlags
     *         mask of modifiers to consider
     * @param editGroup
     *         the edit group in which to collect the corresponding text edits, or
     *         <code>null</code> if ungrouped
     * @return a tracked position that contains the changed modifiers
    ...
    private void internalSetModifiers(int modifiers, int consideredFlags, TextEditGroup editGroup) {
        ...
        // remove modifiers
        List<ASTNode> originalList = fModifierRewrite.getOriginalList();
