...
public class CodeFormatterUtil {
    ...
    /**
     * Creates a string that represents the given number of indentation units. The returned string can contain tabs and/or spaces
     * depending on the core formatter preferences.
     *
    ...
    public static int getTabWidth() {
      /*
       * If the tab-char is SPACE, FORMATTER_INDENTATION_SIZE is not used by the core formatter. We piggy back the visual tab
       * length setting in that preference in that case.
       */
        String key;
        if (JavaCore.SPACE.equals(getCoreOption(DefaultCodeFormatterConstants.FORMATTER_TAB_CHAR)))
            key = DefaultCodeFormatterConstants.FORMATTER_INDENTATION_SIZE;
        else
            key = DefaultCodeFormatterConstants.FORMATTER_TAB_SIZE;

        return getCoreOption(key, 4);
    }
    ...
    public static int getIndentWidth() {
        String key;
        if (DefaultCodeFormatterConstants.MIXED.equals(getCoreOption(DefaultCodeFormatterConstants.FORMATTER_TAB_CHAR)))
            key = DefaultCodeFormatterConstants.FORMATTER_INDENTATION_SIZE;
        else
            key = DefaultCodeFormatterConstants.FORMATTER_TAB_SIZE;
     ...
     * @since 3.1
     */
    private static int getCoreOption(String key, int def) {
        try {
            return Integer.parseInt(getCoreOption(key));
     ...
     *
     * @param kind
     *         Use to specify the kind of the code snippet to format. It can be any of the kind constants defined in
     *         {@link CodeFormatter}
     * @param source
     ...
     *         one will be used.
     * @param options
     *         The options map to use for formatting with the default code formatter. Recognized options are documented on
     *         {@link JavaCore#getDefaultOptions()}. If set to <code>null</code>, then use the current settings from
     *         {@link JavaCore#getOptions()}.
     * @return the formatted source string
     */
    public static String format(int kind, String source, int indentationLevel, String lineSeparator,
     ...
     */
    public static TextEdit format2(int kind, String source, int offset, int length, int indentationLevel,
                                   String lineSeparator, Map<String, String> options) {
        if (offset < 0 || length < 0 || offset + length > source.length()) {
            throw new IllegalArgumentException(
                    "offset or length outside of string. offset: " + offset + ", length: " + length + ", string size: " +
                    source.length()); //$NON-NLS-1$//$NON-NLS-2$//$NON-NLS-3$
        }
        return ToolFactory.createCodeFormatter(options).format(kind, source, offset, length, indentationLevel,
                                                               lineSeparator);
    }
     ...
     *         If the offset and length are not inside the string, a IllegalArgumentException is thrown.
     */
    public static TextEdit format2(int kind, String source, int indentationLevel, String lineSeparator,
                                   Map<String, String> options) {
        return format2(kind, source, 0, source.length(), indentationLevel, lineSeparator, options);
    }

    /**
     * Creates edits that describe how to re-format the given string. This method should be used for formatting existing code.
    ...
    /**
     * Creates edits that describe how to re-format the regions in the given string. This method should be used for formatting
     * existing code. Returns <code>null</code> if the code could not be formatted for the given kind.
     * <p/>
     * <p>
     ...
     *
     * @param kind
     *         Use to specify the kind of the code snippet to format. It can be any of K_EXPRESSION, K_STATEMENTS,
     ...
     * @since 3.4
     */
    public static TextEdit reformat(int kind, String source, Region[] regions, int indentationLevel,
                                    String lineSeparator, Map<String, String> options) {
        return ToolFactory.createCodeFormatter(options, ToolFactory.M_FORMAT_EXISTING).format(kind, source, regions,
     ...
     *         If the offset and length are not inside the string, a IllegalArgumentException is thrown.
     */
    public static TextEdit reformat(int kind, String source, int indentationLevel, String lineSeparator,
                                    Map<String, String> options) {
        return reformat(kind, source, 0, source.length(), indentationLevel, lineSeparator, options);
    }

    /**
     * Creates edits that describe how to format the given string. The given node is used to infer the kind to use to format the
     * string. Consider to use {@link #format2(int, String, int, String, Map)} if the kind is already known. Returns
     ...
     *         If the offset and length are not inside the string, a IllegalArgumentException is thrown.
     */
    public static TextEdit format2(ASTNode node, String source, int indentationLevel, String lineSeparator,
                                   Map<String, String> options) {
        int code;
        ...
        TextEdit edit =
                format2(code, concatStr, prefix.length(), source.length(), indentationLevel, lineSeparator, options);
