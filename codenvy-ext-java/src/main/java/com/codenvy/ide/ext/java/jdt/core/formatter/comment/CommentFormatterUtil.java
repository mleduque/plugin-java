 ...
 *******************************************************************************/
package com.codenvy.ide.ext.java.jdt.core.formatter.comment;
...
public class CommentFormatterUtil {
     ...
     *         if the positions are not inside the string
     */
     ...
     *
     * @throws IllegalArgumentException
     *         if the offset and length are not inside the string
     */
    public static TextEdit format2(int kind, String string, int indentationLevel, String lineSeparator, Map options) {
        ...
        if (0 < 0 || length < 0 || 0 + length > string.length()) {
            throw new IllegalArgumentException(
                    "offset or length outside of string. offset: " + 0 + ", length: " + length + ", string size: " +
                ...
                doc.addPositionUpdater(new DefaultPositionUpdater(POS_CATEGORY) {
                    protected boolean notDeleted() {
                        if (this.fOffset < this.fPosition.offset
                            && (this.fPosition.offset + this.fPosition.length < this.fOffset + this.fLength)) {
                ...
                for (int i = 0; i < positions.length; i++) {
                    try {
                        ...
                        throw new IllegalArgumentException(
                                ...
                                ", string size: " + content.length()); //$NON-NLS-1$//$NON-NLS-2$//$NON-NLS-3$
                    }
                }
            }
