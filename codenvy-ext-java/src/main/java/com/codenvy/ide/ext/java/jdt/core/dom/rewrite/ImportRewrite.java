 ...
 * {@link com.codenvy.ide.ext.java.jdt.core.dom.rewrite.ASTRewrite} as long as no import statements are modified by the AST rewrite.
 * </p>
 * <p>
...
public final class ImportRewrite {
    ...
    public static abstract class ImportRewriteContext {
         ...
         *
         * @param qualifier
         *         The qualifier of the element, can be package or the qualified name of a type
         * @param name
         *         The simple name of the element; either a type, method or field name or * for on-demand imports.
         * @param kind
         *         The kind of the element. Can be either {@link #KIND_TYPE}, {@link #KIND_STATIC_FIELD} or
    ...
    private final Map<String, Object> importsKindMap;
     ...
     */
    public static ImportRewrite create(Document document, CompilationUnit astRoot, boolean restoreExistingImports) {
        if (astRoot == null) {
    ...
    private ImportRewrite(Document document, CompilationUnit astRoot, List<String> existingImports) {
        ...
        this.importsKindMap = new HashMap<String, Object>();
    }
     ...
     *
     * @param threshold
     *         a positive number defining the on-demand import threshold for normal (non-static) imports.
     * @throws IllegalArgumentException
     *         a {@link IllegalArgumentException} is thrown if the number is not positive.
     */
     ...
     * </p>
     *
     * @param useContextToFilterImplicitImports
     *         the given setting
   ...
   /* package */
     ...
     *         the AST to create the returned type for.
     * @param context
     *         an optional context that knows about types visible in the current scope or <code>null</code> to use the
    ...
    public Type addImport(ITypeBinding binding, AST ast, ImportRewriteContext context) {
        ...
        if (qualifiedName.length() > 0) {
            ...
            if (typeArguments.length > 0) {
                ...
                List<Type> arguments = paramType.typeArguments();
                for (int i = 0; i < typeArguments.length; i++) {
                    ITypeBinding curr = typeArguments[i];
                    if (containsNestedCapture(curr, false)) { // see bug 103044
     ...
     * @throws IllegalArgumentException
     *         an {@link IllegalArgumentException} is thrown if the binding is not a static field or
     *         method.
     */
     ...
     *         The binding of the static field or method to be added.
     * @param context
     *         an optional context that knows about members visible in the current scope or <code>null</code> to use the
     *         default context only using the available imports.
     * @return returns either the simple member name if the import was successful or else the qualified name if an import conflict
     *         prevented the import.
     * @throws IllegalArgumentException
    ...
    public final TextEdit rewriteImports() {
        ...
        if (usedAstRoot == null) {
            ...
            usedAstRoot = (CompilationUnit)parser.createAST();
        }
        ...
        ImportRewriteAnalyzer computer =
                new ImportRewriteAnalyzer(document, usedAstRoot, this.importOrder, this.importOnDemandThreshold,
                                          this.staticImportOnDemandThreshold, this.restoreExistingImports,
        ...
        try {
            result = computer.getResultingEdits();
