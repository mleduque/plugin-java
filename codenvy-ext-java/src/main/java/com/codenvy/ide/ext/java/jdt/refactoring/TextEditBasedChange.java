...
public abstract class TextEditBasedChange extends Change {
    ...
    static final class LocalTextEditProcessor extends TextEditProcessor {
        ...
        protected LocalTextEditProcessor(Document document, TextEdit root, int flags) {
    ...
    static final class PreviewAndRegion {
        ...
        public PreviewAndRegion(Document d, Region r) {
            document = d;
            region = r;
        }
    }
     ...
     *
     * @param name
     *         the name of the text edit change
    ...
    String getContent(Document document, Region region, boolean expandRegionToFullLine, int surroundingLines)
        ...
        try {
            if (expandRegionToFullLine) {
                ...
                if (region.getLength() == 0) {
                    ...
                    endLine =
                            Math.min(document.getLineOfOffset(region.getOffset()) + surroundingLines - 1,
                                     document.getNumberOfLines() - 1);
                } else {
                    endLine =
                            Math.min(document.getLineOfOffset(region.getOffset() + region.getLength() - 1) + surroundingLines,
                                     document.getNumberOfLines() - 1);
                }

                int offset = document.getLineInformation(startLine).getOffset();
                Region endLineRegion = document.getLineInformation(endLine);
...
//            Log.error(getClass(), e);
     ...
     */
    public abstract String getCurrentContent() throws CoreException;
     ...
     *
     * @param region
     *         the starting region for the text to be returned
     ...
     */
    public abstract String getCurrentContent(Region region, boolean expandRegionToFullLine, int surroundingLines) throws CoreException;
     ...
     */
    public abstract String getPreviewContent(TextEditBasedChangeGroup[] changeGroups, Region region,
     ...
     */
    public abstract String getPreviewContent() throws CoreException;
     ...
     *
     * @param type
     *         the text type. If <code>null</code> is passed the text type is
